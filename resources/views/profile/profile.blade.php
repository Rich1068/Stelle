...
@else 
    {{ $user->description }} 
@endif </h3><br><br>
<h3><b>Information</b> <br> 
Email: {{ $user->email }} <br> 
Contact Number: @if ($user->contact_number === null) N/A @else {{ $user->contact_number }} @endif 
<br> Country: @if($user->country_id === null) N/A @else {{ $countryTable->countryname }} <img src="{{ asset("storage/images/flags/{$countryTable->code}.png") }}" alt="Profile pic" style="max-width: 50px; max-height: 50px;"> @endif</h3>
